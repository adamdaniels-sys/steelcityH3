...
4. **Homepage photo slideshows.** ✅ DONE (2026-05-22).
   - The hero photo frame is a single cycling slideshow, plus **three framed
     slideshow bands spaced down the page** (`.snap-band` before three of the
     soundfx dividers), so scrolling reveals snaps from past hashes. Each pulls
     random photos from all uploaded event photos on its own staggered timer
     (preloaded crossfade, decorative/no click). Hero falls back to the "landing
     here soon" placeholder and the bands stay hidden when there are no photos.
     Needs `0022` + uploads.
